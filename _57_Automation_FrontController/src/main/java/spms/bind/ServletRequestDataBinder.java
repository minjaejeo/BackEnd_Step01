...
public class ServletRequestDataBinder {
	...
	public static Object bind(
		...
		for(String paramName : paramNames) {
			...
			if(m != null) {
				// 메서드 정보를 가지고 메서드를 호출
				// dataObject로부터 m에 저장된 setter 메서드를 호출해서 값을 저장한다.
				// m.getParameterTypes()[0] - setter 메서드의 매개변수 타입
	...
	private static Object createValueObject(Class<?> type, String value) {
		...
		else if(type.getName().equals("double") || type == Float.class)
			return Double.valueOf(value);
		else if(type.getName().equals("long") || type == Float.class)
			return Long.valueOf(value);
		else if(type.getName().equals("boolean") || type == Float.class)
